...
async def run_agent():
    ...
    try:
        async with async_playwright() as p:
            ...
            # Extract top 3 articles directly using DOM evaluation
            articles = await page.evaluate("""() => {
                const rows = Array.from(document.querySelectorAll('.athing')).slice(0, 3);
                return rows.map(row => {
                    ...
                    return {
                        ...
                        summary: "Trending discussion on Hacker News today."
                    };
                });
            }""")
